...
let transactionHistory: Array<{ ticker: string, action: string, shares: number, price: number, date: string }> = [];

// Local memory trade executor function - completely eliminates loopback HTTP request issues
const executeTradeInMemory = async (ticker: string, action: string, shares: number, price: number) => {
  ...
  if (action === 'BUY') {
    if (userPortfolio.balance >= cost) {
      ...
      transactionHistory.push({ ticker, action, shares, price, date: new Date().toISOString() });
      console.log(`✅ [SERVER] Bought ${shares}x ${ticker}. Balance: $${userPortfolio.balance.toFixed(2)}`);
      return { success: `Successfully executed: BUY ${shares} shares of ${ticker} at $${price.toFixed(2)}.` };
    } else {
    ...
    if (pos && pos.shares >= shares) {
      ...
      transactionHistory.push({ ticker, action, shares, price, date: new Date().toISOString() });
      console.log(`✅ [SERVER] Sold ${shares}x ${ticker}. Balance: $${userPortfolio.balance.toFixed(2)}`);
      return { success: `Successfully executed: SELL ${shares} shares of ${ticker} at $${price.toFixed(2)}.` };
    } else {
...
// Expose history for inspection if needed
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    // Pass transactionHistory and the direct callback trade executor function to avoid any loopback network calls
