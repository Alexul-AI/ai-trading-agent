...
- Uses raw Alpaca daily bars (adjustment=raw) by default. ETF dividends/distributions (SPY/EFA/TLT all pay meaningful yields; GLD does not) are not included, so both the strategy's and the benchmarks' returns are price-return, not total-return. The relative comparison between strategy and benchmark is still apples-to-apples (both miss dividends the same way), but the absolute return numbers above are not real total-return figures, and "beats SPY" should be read as "beats SPY's raw price return," not necessarily its total return. **Update (2026-08-08):** the research this caveat asked for has happened (\`backtest-etf-rotation-adjustment-comparison.ts\`, PRs #76/#77) - \`adjustment=all\` changes ETF Rotation's picks on 10.7-13.1% of rebalance dates, with a pre-declared-criteria verdict of "adjusted likely better." Still research only - the live default and this script's own default both remain \`raw\` pending a separate, explicit decision (see \`docs/product/ROADMAP.md\` Phase 2's risk-tiered plan). Pass \`adjustment: "all"\` to \`runEtfRotationWindowAnalysis\` to re-run any specific comparison under adjusted data.
...
// imported for runEtfRotationWindowAnalysis - without this guard, every
// script that imports from here (multiwindow, holdcount-sweep, out-of-sample)
// silently also re-ran this file's own main() on import: an extra Current-
// window fetch/simulation/report-write on every single run of those other
// scripts, and confusing interleaved console output. Same fix applied to
// backtest-portfolio.ts, which has the identical pattern.
