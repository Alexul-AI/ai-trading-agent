...
2. **Research infra** — forward-validation (`backtest-etf-rotation-forward-validation.ts`) можно расширить, чтобы параллельно с существующим raw-трекингом (`baseline-2`/`candidate-hold3`, см. выше) вести **отдельный** adjusted-shadow-трекинг тех же двух config-вариантов под `adjustment=all`. **Критично: это отдельные часы, не замена существующих** — raw-трекинг уже накопил `rebalance_count=2` с 2026-07-14 и не должен сбрасываться; adjusted-shadow-трекинг стартует свои часы с нуля с даты своего первого реального запуска, с тем же порогом чтения (3+/6+ циклов), но отдельно посчитанным. Сентябрьское чтение должно явно указывать, к какому треку (raw prod-config или adjusted-shadow) относится каждое число, а не сливать их в одну оценку. Ещё не реализовано — это дизайн для будущего PR, не код.
