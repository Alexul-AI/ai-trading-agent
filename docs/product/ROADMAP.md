...
**Обновление (2026-07-14): forward-валидация запущена — `backtest-etf-rotation-forward-validation.ts`.** Реализация по указанию пользователя: без нового live-воркера, без production shadow-portfolio сервиса — переиспользует существующий backtest pipeline (`runEtfRotationWindowAnalysis`). Механика: baseline-2 и candidate-hold3 гоняются на том же стандартном окне "Current (~900d)", затем метрики пересчитываются **только по срезу после якорной даты** `FORWARD_VALIDATION_ANCHOR_DATE = "2026-07-14"` (дата, когда candidate-hold3 был официально назван и историческая проверка признана исчерпанной) — return и max drawdown пересчитаны с нуля от точки на якоре, отдельно от общих чисел по всему окну. Каждая сделка с этой даты сохраняется (ticker/date/action/shares/price + приблизительный realized weight%). SPY/equal-weight считаются отдельным лёгким fetch за тот же период, как контекст (не полноценная rebased-симуляция). Никаких изменений в `backtest-etf-rotation.ts`, `etfRotationStrategy.ts` или live/paper execution path.

**Критерии чтения результата зафиксированы заранее** (до того как появились любые forward-данные): 0 ребалансировок с якоря — читать нечего; 1-2 ребалансировки (~1-2 месяца) — цифры показываются, но помечены как "слишком рано для решения о promotion"; 3+ ребалансировки (~3 месяца, по собственной оценке пользователя) — candidate-hold3 считается "держится нормально", только если ОБА условия выполнены: (a) forward max drawdown не хуже, чем у baseline-2, и (b) forward return не хуже более чем на 5 п.п. Невыполнение любого из условий — повод для дальнейшего анализа, не автоматический отказ. В любом случае это дополнительный контекст поверх уже завершённой исторической multi-window проверки, а не её замена, и само по себе не запускает promotion в `DEFAULT_ETF_ROTATION_CONFIG`.

Результат — накапливаемый CSV-лог (`etf-rotation-forward-validation-log.csv`, строка добавляется при каждом запуске, не перезаписывается) плюс отчёт за последний прогон. Первый реальный запуск (2026-07-14, сам день якоря): 1 торговый день, 0 ребалансировок, все forward-показатели 0.00% — скрипт корректно сообщает "читать пока нечего", не выдавая снимок одного дня за находку. Повторный запуск в той же сессии подтвердил, что CSV-лог действительно добавляет вторую строку, а не перезаписывает первую.
