...
**Обновление (2026-07-15): реальная pre-flight проверка нашла блокирующий пробел — ETF Rotation не подключена ни к одному live-пути.** `GET /api/autopilot/status` (реальный запрос к живому Render-деплою) вернул `strategyVersion: "v1.2-confluence-scoring"` и 13-ticker baseline universe — не 5-ETF Rotation universe. Проверка кода подтверждает: `autopilotWorker.ts` нигде не импортирует и не вызывает `etfRotationStrategy.ts`/`decideRotationTargets` — вся работа над ETF Rotation этого проекта (MVP, multi-window, hold-count sweep, forward validation) существует только в standalone backtest-скриптах, не в live/autopilot коде. **Это значит: заголовок этой фазы ("Paper Execution — ETF Rotation only") пока не может быть выполнен даже при полностью готовой инфраструктуре** — сначала нужна отдельная, ещё не спроектированная интеграционная работа (как месячная ребалансировка ETF Rotation вписывается в hourly-cycle архитектуру `autopilotWorker.ts`, какой config-вариант использовать — `baseline-2` или `candidate-hold3`, заменяет ли это baseline-стратегию полностью или это toggle, как это соотносится с bucket-cap/circuit breaker, спроектированными вокруг 13-ticker universe). Явное решение пользователя: не включать execution сейчас, не торговать baseline "случайно", ETF Rotation integration — следующая крупная задача, требующая отдельного Plan Mode раунда, не быстрый follow-up. Попутно проверен реальный paper-аккаунт (`GET /api/portfolio`, `/api/orders`, `/circuit-breaker/review`) — чист, без зависших позиций от инцидента 2026-06, equity ~$88,131 в кэше. Также найдено: `AUTOPILOT_ALLOW_BUY`/`AUTOPILOT_ALLOW_SELL` — отдельные гейты помимо `AUTOPILOT_EXECUTE_TRADES`, сейчас оба `false`; `docs/ops/PAPER_EXECUTION_START_PLAN.md` обновлён с учётом этого. Полный лог проверки — в том же документе, раздел "Verification log".

**Обновление (2026-07-15): Stage 1 ETF Rotation live-integration закрыт — decision-only, execution по-прежнему не подключена.** Прямой ответ на пробел из предыдущего обновления. Дизайн: `AUTOPILOT_STRATEGY` env var (`baseline` default | `etf_rotation`) делает стратегии **взаимоисключающими**, никогда не работающими одновременно — обходит стороной проблему, которую отдельный research (два параллельных Explore-агента) выявил как самую острую: 13-ticker baseline universe и 5-ETF universe пересекаются (SPY/EFA/TLT/GLD), а `orderIdempotency.ts`'s `client_order_id`-трекер (единственный на процесс, keyed только `"TICKER:ACTION"`) и вся позиционная модель (Alpaca positions, bucket cap — всё keyed только по тикеру, без strategy attribution) не смогли бы безопасно разделить два одновременно рассуждающих о одном и том же тикере стратегии. Реализовано (`autopilotWorker.ts`, `etfRotationStrategy.ts`, новый `etfRotationWorkerState.ts`): месячный rebalance-gate с персистентным состоянием (`data/etf-rotation-worker-state.json`, тот же fail-soft `filePath`-паттерн, что и у circuit breaker/order-idempotency); вычисление rebalance-целей на реальных live-барах через уже существующие `decideRotationTargets`/`resolveEtfRotationConfigVariant` (переиспользует PR #29's config-variant механизм без изменений); перевод целей в конкретные BUY/SELL-ордера (`computeRebalanceOrders`, полный liquidate-then-rebuy, SELL перед BUY); журналирование через расширенную (optional-fields) `JournalDecision`/`AutopilotDecisionLog`-схему с новым `reasonType`-словарём (`REBALANCE_BUY`/`REBALANCE_SELL`/`REBALANCE_HOLD`/`NOT_REBALANCE_DAY`). **Жёсткая гарантия на уровне кода, не просто флаг**: rotation-путь ни разу не вызывает `executeSafeTrade` — каждое вычисленное решение журналируется с `executionStatus: "dry_run"` (BUY/SELL) или `"not_attempted"` (HOLD) безусловно, независимо от `AUTOPILOT_EXECUTE_TRADES`. `getStatus()`/`appendAutopilotRun` теперь условно репортят активную стратегию (`strategyVersion`/`tickers`/`strategyConfig`) — важно конкретно потому, что `docs/ops/PAPER_EXECUTION_START_PLAN.md`'s pre-flight checklist инструктирует проверять именно это поле для подтверждения identity стратегии. **Проверено сквозь реальные live-данные** (изолированный verification-run против настоящего paper-аккаунта, не мок): rebalance-day корректно определён на первом запуске, `decideRotationTargets` выбрала реальные топ-2 (SPY/QQQ, оба по 50%) с реалистичными объёмами (58/61 акций на ~$88k эквити), второй запуск в тот же месяц корректно вернул `NOT_REBALANCE_DAY` без пересчёта, `executeSafeTrade`-заглушка (специально настроенная бросать ошибку при вызове) ни разу не сработала — подтверждает нулевой execution. Baseline-путь (`AUTOPILOT_STRATEGY` unset) проверен отдельно и ведёт себя идентично прежнему поведению — default-safe подтверждён, а не предположен. Typecheck/tests зелёные (199/199). Stage 2 (реальное исполнение ордеров через тот же `executeSafeTrade`/`evaluateTrade`/bucket-cap/circuit-breaker chain, за теми же тремя env vars) и Stage 3 (полировка — `QQQ` в `TICKER_TO_BUCKET`, финальные doc-обновления) намеренно отложены, не начаты.
