...
**Методология исправлена до мерджа (поймано в review пользователя, не самостоятельно)**: первая версия гоняла стандартное окно "Current (~900d)" (симуляция с ~2024-11-25) и *резала* equity curve каждого конфига по якорной дате — то есть на якорную дату оба конфига уже держали какие-то позиции, выбранные ~20 месяцами ДО-якорной истории ребалансировок, и могли отличаться друг от друга просто из-за разного пути ДО якоря, а не из-за чего-либо после него. Исправлено без изменения shared sim engine: `isMonthlyRebalanceDate` в `etfRotationStrategy.ts` уже возвращает `true` безусловно на самый первый симулированный день. Подобрав размер fetch-окна так, чтобы естественная точка старта симуляции (после прогрева индикаторов) оказалась близко к якорю, симуляция теперь честно стартует с чистого кэша и сразу исполняет первую ребалансировку — одинаково для обоих конфигов, без взаимного заражения состоянием. Точное попадание в якорную дату невозможно (округление календаря/warmup-буфера) — реальный разрыв считается и печатается явно, а не растушёвывается словами "близко к": первый реальный прогон дал разрыв 26 календарных дней (старт 2026-06-18 против якоря 2026-07-14) — на порядок меньше и симметрично для обоих конфигов, в отличие от исходных ~20 месяцев заражения.
...
Результат — накапливаемый CSV-лог (`etf-rotation-forward-validation-log.csv`, строка добавляется при каждом запуске, не перезаписывается) плюс отчёт за последний прогон. Первый реальный запуск под исправленной методологией (окно 2026-06-18 → 2026-07-14, 17 торговых дней, оба конфига сразу разворачивают капитал в день один, затем вторая ребалансировка на границе месяца 2026-07-02 — итого 2 ребалансировки): baseline-2 −1.07% (maxDD −3.33%), candidate-hold3 −0.74% (maxDD −2.87%), против SPY +0.81% и equal-weight-5-ETF −1.41% за тот же период — корректно прочитано как "2 ребалансировки, слишком рано для решения о promotion, только для информации", не как находка. Повторный запуск в той же сессии подтвердил, что CSV-лог добавляет строку, а не перезаписывает.
