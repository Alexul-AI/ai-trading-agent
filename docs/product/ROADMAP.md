...
**Читать осторожно**: это одно окно, один прогон — ещё не прошло multi-window validation, которая является собственным стандартом проекта (Phase 1) перед тем как считать что-либо реальной находкой. Multi-window companion — естественный следующий шаг, та же поэтапная схема, что и с `backtest-portfolio.ts` (сначала single-window, потом multi-window после того как появились реальные числа, оправдывающие эту работу).
