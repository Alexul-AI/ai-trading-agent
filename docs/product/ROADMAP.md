...
Результат — накапливаемый CSV-лог (`etf-rotation-forward-validation-log.csv`, строка добавляется при каждом запуске, не перезаписывается) плюс отчёт за последний прогон. Первый реальный запуск под полностью исправленной методологией: окно 2026-07-14 → 2026-07-14 (сегодня, сам якорь — попадание точно в него, без единого ДО-якорного дня), 1 торговый день, 1 ребалансировка для обоих конфигов (немедленное развёртывание капитала с чистого cash), все результаты 0.00% — время ещё не прошло. Решение NEXT_OPEN-модели в день один пока не имеет следующего дня для исполнения (`finalRebalanceUnexecuted`, уже существовавший, документированный краевой эффект) — и консоль, и отчёт теперь явно об этом говорят, а не молча показывают "1 ребалансировка" рядом с пустым списком решений. Корректно прочитано как "слишком рано для решения о promotion, только для информации". Повторные запуски в той же сессии (через оба раунда исправлений) подтвердили, что CSV-лог добавляет строку, а не перезаписывает. По состоянию на 2026-08-08: `rebalance_count=2` (2026-07-14 день-1 + начало августа) — следующая ребалансировка (и, соответственно, `rebalance_count=3`, первый порог чтения) произойдёт только в начале сентября.

**Обновление (2026-08-08): `adjustment=raw` vs `adjustment=all` research (PR #76, #77) — вердикт "adjusted likely better", но live не тронут; это decision-план, не live-изменение.** Внешний review указал, что `adjustment=raw` (дефолт везде — и в live, и во всех backtest-скриптах) не вычитает дивиденды/дистрибуции из цены, что теоретически могло искажать 126-дневный momentum-ranking. Проверено двумя research-only PR (никаких изменений в `alpacaBarsFetch.ts`/`autopilotWorker.ts`/`server.ts`/env/ramp/repair endpoint ни в одном из них):

- **PR #76**: прямой Alpaca API тест подтвердил, что `adjustment=all` реально поддерживается этим аккаунтом (IEX feed) и даёт другие цены, чем `raw` (TLT: разница ~$0.3-0.7 на реальной дистрибуции 2026 года). На тех же 5 стандартных окнах, что и остальной research этого проекта: **11 из 84 ребаланс-дат (13.1%) меняют top-2 picks** между `raw` и `all` (только `baseline-2`).
- **PR #77 (deep-dive)**: то же самое, но на обоих config-вариантах (`baseline-2` и `candidate-hold3`) плюс full momentum-delta таблица по каждому тикеру на каждой дате (не только на датах с flip). `candidate-hold3` даёт меньший, но всё ещё реальный эффект (7/84, 8.3%). Momentum-дельты чисто упорядочены по известной дивидендной доходности: TLT +1.60pp, EFA +1.51pp, SPY +0.74pp, QQQ +0.35pp, **GLD +0.00pp ровно** (GLD не платит дистрибуций — сильная внутренняя проверка, что измерение действительно ловит нужный эффект). **Вердикт по заранее зафиксированным критериям (materiality >= 5%, механизм подтверждён — payers >= 2x non-payers по |delta|, Calmar-большинство >= 60%): "adjusted likely better"** — materiality 10.7% (оба варианта вместе), механизм подтверждён (1.603pp vs 0.353pp), Calmar `all` >= `raw` в 16 из 20 комбинаций window×variant×model (80%).

**Это исследовательский вывод, не production-решение.** Пользователь явно разделил дальнейшую работу на risk-tiers, до перехода к любому live-изменению:

1. **Low-risk (можно раньше)** — обновить wording в старых reports/docs: `raw` = price-return метрика, `adjustment=all` вероятно методологически лучше, к историческим raw-числам в этом проекте следует относиться с этой оговоркой. Не меняет поведение, только текст.
2. **Research infra** — forward-validation (`backtest-etf-rotation-forward-validation.ts`) можно расширить, чтобы параллельно с существующим raw-трекингом (`baseline-2`/`candidate-hold3`, см. выше) вести **отдельный** adjusted-shadow-трекинг тех же двух config-вариантов под `adjustment=all`. **Критично: это отдельные часы, не замена существующих** — raw-трекинг уже накопил свои циклы и не должен сбрасываться; adjusted-shadow-трекинг стартует свои часы с нуля с даты своего первого реального запуска, с тем же порогом чтения (3+/6+ циклов), но отдельно посчитанным. Сентябрьское чтение должно явно указывать, к какому треку (raw prod-config или adjusted-shadow) относится каждое число, а не сливать их в одну оценку. **Реализовано (PR #79, 2026-08-08)**: `runForwardValidationTrack` (extracted из существующего скрипта, raw-трек — тонкая обёртка вокруг неё, byte-identical output подтверждён) + новый `backtest-etf-rotation-forward-validation-adjusted-shadow.ts`, свой report/CSV, анкер `2026-08-08`, тот же еженедельный workflow. Отчёт явно помечен "methodology evidence, not production clearance" — сам по себе не триггерит live-изменение.
3. **Production-adjacent (отдельный PR, явный сигнал)** — переключение live ETF Rotation bars fetch (`alpacaBarsFetch.ts`) на `adjustment=all` меняет реальные торговые решения стратегии, а не просто описание. Требует отдельного PR с явной формулировкой "this changes strategy decisions", отдельного явного сигнала пользователя — не бандлится ни с каким другим PR. **Обязательное требование к дизайну этого будущего PR (добавлено 2026-08-08, по итогам аудита barrier/rate-limit/adjustment-рисков после PR #80): dual-price pipeline, не единая замена цены.** Сегодня live-код везде читает `adjustment=raw` — и для momentum/SMA-сигналов, и (косвенно, через тот же bars-фетч) для всего остального; переключение на `all` не должно быть таким же единым свопом в обратную сторону. `adjustment=all` уместен для сигналов/ranking (`decideRotationTargets`), но sizing (`computeRebalanceOrders`'s `targetDollars`/`Math.floor(.../price)`, и любой будущий notional-путь) обязан использовать raw/real-time цену — adjusted close систематически отличается от того, по чему реально исполнится market-ордер на бирже, и ошибка в sizing (не только в лимит-цене, которую ETF Rotation сегодня не использует вообще) — это неверное количество акций/notional-сумма, а не просто риск неисполнения лимитника. Этот PR обязан явно развести, какие вызовы читают `all`, а какие — `raw`/realtime quote, а не просто сменить один параметр глобально.
4. **Не трогать до сентября** — `AUTOPILOT_ETF_ROTATION_RAMP_MAX_POSITION_PERCENT`, любой другой config/env, `POST /api/autopilot/etf-rotation/repair-missing-buy` endpoint.

**Обновление (2026-08-08, PR #82): Sharpe/Sortino/turnover/tax-drag доставлены, но не как часть Phase 1's scorecard.** Отдельный, изолированный модуль `riskTaxMetrics.ts` (не расширение `scorecard.ts` — намеренно, чтобы не задеть уже опубликованные отчёты) даёт annualized volatility, Sharpe, Sortino, gross/annualized turnover, average holding period, realized gain/loss и illustrative tax-drag сценарии (0/15/25/35%, явно помечено "not tax advice"). Wired только в новый research-скрипт (`backtest-etf-rotation-methodology-metrics.ts`), не в существующие отчёты — существующие числа (CAGR/max DD/Calmar) остаются как были.

**Обновление (2026-08-08 — 2026-08-11, PR #83/#84/#85): исследован механизм full-liquidate-then-rebuy — backtest обнадёживает, live-данные показывают, что выгода зависит от ramp cap.** `computeDeltaRebalanceOrders` (PR #83, новая функция, торгует только дельту к target вместо полного round-trip) в backtest дал **55-85% снижения turnover / ~60-75% меньше сделок** на 5 окнах × 2 config-варианта × 4 порога `[0,1,2,5]%`, при строго доказанной (unit-тестом, не симуляцией) алгебраической идентичности результата с full-liquidate при `threshold=0`. PR #84 — docs-only дизайн того, что потребовалось бы для live-подключения (новый `legType`-словарь, audit-событие для threshold-skip, drift-aware repair) — намеренно без реализации, и явно зафиксировал: выгода delta-only и решение про ramp cap — **не независимые вопросы**. PR #85 проверил это на реальных данных: read-only daily shadow (`etf-rotation-delta-only-live-shadow.ts` + GH Actions workflow) сравнивает, что сделал бы delta-only *в этот конкретный цикл* против реального портфеля и реальных целей (через уже публичный `GET /api/autopilot/etf-rotation/review`, без изменений в `server.ts`). **Первое реальное наблюдение (ребаланс "2026-08") дало только 7.2% снижения** — далеко от 55-85% из бэктеста — потому что реальный портфель сейчас далеко от целевых весов (QQQ на 0% из-за августовского инцидента, SPY на 1.75% вместо 50% из-за live ramp cap 2%): когда портфель далеко от target, дельта почти равна полному target, экономить почти нечего. Это не противоречие PR #83, а живое подтверждение PR #84's пункта 4. Ничего в live/execution/env не менялось ни в одном из трёх PR — пользователь явно решил не "чинить" delta-only сейчас, а дать daily shadow понаблюдать до сентябрьского ребаланса.
...
**Обновление (2026-08-08, синхронизация документа с реальностью — этот раздел отстал от кода на три недели): Stage 2 на самом деле уже запущен, начиная примерно с 2026-07-17.** `AUTOPILOT_EXECUTE_TRADES=true`/`AUTOPILOT_ALLOW_BUY=true` включены в Render, `AUTOPILOT_STRATEGY=etf_rotation` — активная live-стратегия. Реальная история ордеров подтверждает: первые `open_new` BUY по SPY/QQQ прошли 2026-07-17. **2026-08-03 состоялась первая настоящая ребалансировка с реальными SELL-легами** (`AUTOPILOT_ALLOW_REBALANCE_SELLS` включён отдельным гейтом от baseline-стратегии's `AUTOPILOT_ALLOW_SELL`) — SPY успешно переисполнилась (SELL+BUY), но QQQ's rebuild BUY был отклонён Alpaca (HTTP 403, same-symbol order-crossing race — SELL ещё не успел исполниться, когда ушёл paired BUY; полный разбор в `CLAUDE.md`, запись от 2026-08-04). По состоянию на 2026-08-08 QQQ так и остаётся не докупленным, позиция помечена off-target, ручного repair не производилось — это осознанное решение, не забытая задача (см. ниже). PR #63–#71 закрыли операционный разрыв, который вскрыл этот инцидент: видимость off-target состояния (`offTargetReview` в `GET /api/autopilot/etf-rotation/review`), ежедневное Telegram-напоминание, пока leg остаётся missing, repair-specific audit/idempotency-механизм, и (PR #71) admin-only, paper-only endpoint `POST /api/autopilot/etf-rotation/repair-missing-buy`, который может дослать именно недостающий leg — построен, но намеренно ни разу не вызван, ждёт отдельного явного решения. Stage 3 (`QQQ` в `TICKER_TO_BUCKET`) остаётся не начатым, но проверено отдельно (2026-08-08): это не активная проблема, потому что `TICKER_TO_BUCKET`/bucket cap используются только baseline-стратегией (`analyzeTicker`), а не `runEtfRotationCycle` — при взаимоисключающих стратегиях и активной `etf_rotation` это чистый tech debt без текущего эффекта, не блокер.

**Обновление (2026-08-24): найден и исправлен активный observability-баг (PR #86), и зафиксирован будущий operating-model принцип для repair.** Проверка августовского QQQ-инцидента (всё ещё partial три недели спустя) вскрыла, что `offTargetReview` в `GET /api/autopilot/etf-rotation/review` тихо самопогасился: детектор читал только последние 20 записей аудит-лога, а собственные ежедневные `OFF_TARGET_REMINDER_SENT`-события вытеснили исходный `ORDER_REJECTED` из этого окна примерно через 20 дней — `offTarget` стал `false`, daily Telegram-напоминание молча перестало приходить (последнее — 22.08). Исправлено чтением, скоупленным по `rebalanceMonthKey` вместо количества записей — детектор больше не может сам себя заглушить. Чисто observability, execution/gates/ramp/repair endpoint не тронуты.

Отдельно зафиксирован продуктовый принцип целевой операционной модели (пользователь, 2026-08-24): **оператор не выставляет ордера и не инициирует обычное восстановление позиции вручную**. Конечная модель — после partial-ребаланса бот сам выполняет ограниченное автоматическое восстановление с повторной проверкой targets, позиций, market hours, circuit breaker, risk gates и idempotency; человек наблюдает и утверждает политику, не отдельные сделки. Сегодняшний ручной `repair-missing-buy` endpoint — полезный аварийный инструмент, но не конечное состояние. Такой auto-repair **нельзя тихо добавить** — он расширяет торговые полномочия так же, как это сделал бы auto-reset circuit breaker, и по этой же причине требует отдельного design-review (ограничение числа попыток, cooldown, свежий sizing на момент попытки, запрет бесконечных retry, обязательный Telegram-аудит) **после** сентябрьского наблюдения за тем, починит ли scheduled-цикл QQQ сам — не раньше, и не как часть текущего observability-фикса.

**Обновление (2026-09-01): сентябрьский цикл прошёл чисто — QQQ куплен, `status: "executed"`, инцидент закрыт без ручного вмешательства.** Реальные ордера (13:46:35-38 UTC): SPY SELL submitted → accepted, только потом QQQ BUY submitted → accepted, только потом SPY's rebuild BUY submitted → accepted — `waitForSellFill` (PR #75) отработал точно так, как задуман, ни один BUY не обогнал подтверждение своего SELL. Обе позиции реальны (QQQ 2 акции @ ~$707.44, SPY 2 акции @ ~$761.89, ramp-capped с 62/57), `offTargetReview` корректно `false` — и это честная `false`, не самопогасший баг из PR #86 (тот фикс уже стоял на проде к этому моменту). Независимо подтверждено дважды: через API и через реальные Telegram-сообщения пользователя — таймстампы совпадают день-в-день после конвертации UTC→Israel (IDT, +3ч). Ручной repair ни разу не использовался — фикс сам справился, ровно как и планировалось. Это 3-й цикл ребалансировки из порога `GOLIVE_CRITERIA.md` gate 2 (июль-старт → август-partial → сентябрь-executed) — "можно начинать читать", ещё не 6+ "надёжный минимум". Ramp cap/fractional/holdCount=3/adjustment-решения этим одним хорошим циклом не разблокированы автоматически — каждое остаётся отдельным явным решением со своей evidence-базой.

**Paper stage requirements:**
- минимум 4–6 недель scheduled paper execution — на 2026-08-08 набралось ~3 недели (с 2026-07-17), продолжать наблюдение минимум до недели 2026-08-14, надёжнее до 2026-08-28;
- **исходный критерий "20-30 closed paper trades, лучше 50+" написан под ежедневную сигнальную стратегию и не подходит месячному ребалансу ETF Rotation** — при 5-8 легах за полный ребаланс раз в месяц этот порог набирался бы 6-12+ месяцев. Для ETF Rotation как текущей live-стратегии действует отдельный критерий: **3+ полных ребаланс-цикла — можно начинать читать результаты, 6+ — надёжный минимум для любого promotion/go-live решения**, то же самое пороговое значение, что уже используется в `backtest-etf-rotation-forward-validation.ts`'s CSV-логе. Исходный критерий остаётся применимым, если когда-нибудь в live попадёт ежедневная стратегия (например, Trend Participation, Phase 5);
- **важная методологическая оговорка (2026-08-08)**: live paper execution сейчас идёт при `AUTOPILOT_ETF_ROTATION_RAMP_MAX_POSITION_PERCENT=2` (2% от equity на позицию, намеренная ramp-up мера) — это НЕ тот же портфель, что симулирует forward-validation (полный вес 50%/holdCount, без ramp cap). На ~$88k paper-аккаунте 2%-сайзинг даёт позиции по ~$1,500-1,600 (~1.7% эквити) — далеко от целевых весов стратегии. Live paper сейчас проверяет execution-механику (submission, гейты, аудит, repair-flow) на символическом размере, а не реальные return/drawdown характеристики стратегии на её целевых весах. Графика/критериев повышения ramp cap со временем пока не существует — отдельный, ещё не решённый вопрос, не совпадающий с capital-tranche гейтами Phase 4. Отдельно: август — partial/off-target месяц (QQQ не докуплен) — любое сентябрьское чтение результатов обязано явно отделять "система работает операционно корректно" от "портфель соответствует целевым весам стратегии", а не смешивать эти два вопроса в одно число;
...
**Реальный, пока не решённый конфликт (добавлено 2026-08-08): ETF Rotation физически не торгуется на этом транше.** Sizing — только целыми акциями (`computeRebalanceOrders`, без fractional/notional-поддержки, в отличие от baseline-стратегии's опционального `AUTOPILOT_ALLOW_FRACTIONAL_SHARES`). При ценах SPY/QQQ около $750-770 и `holdCount=2`'s весе 50% на слот, $250 (и даже $1,000) капитала не хватает даже на 1 целую акцию любого из топ-пиков. Это значит: текущая live-стратегия не может быть развёрнута на первом транше вообще, независимо от того, пройдены ли остальные гейты. Требует отдельного решения до Phase 4 — либо fractional/notional-поддержка для rotation-пути, либо другой universe с более дешёвыми тикерами, либо явное решение начинать live-транш выше $250.

**Обновление (2026-08-08, PR #80) — конфликт теперь измерен, и распадается на два независимых решения.** Построена (off by default, недостижима из live-воркера) fractional/notional-поддержка для `computeRebalanceOrders`/`etfRotationExecution.ts`, плюс реальная симуляция на 160 комбинациях (5 окон x 2 config-варианта x $100/$250/$500/$1000 x ramp on/off x fractional on/off) на живых данных Alpaca. Результат: **без ramp cap** fractional-поддержка полностью решает проблему на $100-250 (stranded-slot rate падает со 100% до 0%, exposure поднимается до ~90-97%). **С текущим live ramp cap (2%)** fractional-поддержка проблему не решает — ramp's dollar-ceiling обрезает fallback-позицию до нескольких долларов (формально не "stranded", но exposure остаётся на уровне 3-4%), а для тикеров, где целые акции и так проходят, ramp's share-based ceiling всё равно округляет до 0. **Значит small-tranche viability требует не одного решения, а двух отдельных: (1) включать ли fractional/notional для ETF Rotation, и (2) менять ли ramp cap и до какого уровня.** Включение только fractional без пересмотра ramp не разблокирует транш. Оба решения заморожены, как и остальной risk surface, минимум до сентябрьского чтения результатов (см. `GOLIVE_CRITERIA.md`'s gate 2). Полная сетка: `backend/data/backtest-reports/etf-rotation/etf-rotation-small-tranche-simulation-report.md` (gitignored, локально).
