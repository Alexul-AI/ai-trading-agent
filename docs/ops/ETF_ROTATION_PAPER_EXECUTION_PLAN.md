...
**PR #46** (this PR) built `etfRotationExecution.ts` - an isolated `executeEtfRotationOrders` adapter, fully dependency-injected (`executeSafeTrade`, `appendAuditEvent`, `refreshPortfolioSnapshot`, `currentPriceByTicker`, `now`) so it never imports from `autopilotWorker.ts` or `server.ts` and is testable with zero real Alpaca calls. **Not imported or called from `autopilotWorker.ts` anywhere** - confirmed via `git diff --stat`, only the two new adapter files changed. Implements: SELL-legs-before-BUY-legs sequencing; per-leg `AUTOPILOT_ALLOW_BUY`/`AUTOPILOT_ALLOW_SELL` gates (a disallowed SELL also blocks its own paired BUY, same as a SELL that actually failed - not rebuilding a position whose liquidation never happened, whether by failure or by deliberate gate); a global `AUTOPILOT_EXECUTE_TRADES` off-switch that blocks every leg uniformly (distinguished in the result as `"not_attempted"` vs. per-leg `"blocked"`); cash-aware BUY resizing against a *refreshed* portfolio snapshot taken after all SELL legs settle, with a running cash pool decremented across multiple BUY legs in the same cycle (so a second BUY can't double-spend cash the first one already committed); `ORDER_SUBMITTED`/`ORDER_FILLED`/`ORDER_REJECTED`/`ORDER_AMBIGUOUS` audit events for every *attempted* leg only (a blocked/never-attempted leg writes no audit event - the returned result's `blockedOrders` array is what a caller journals for those, keeping "what the adapter tried" separate from "what was blocked before trying"); a pure `computeOverallExecutionStatus` (directly tested for all six reachable values) that treats any ambiguous leg as the whole cycle's status regardless of what else succeeded, per §8's "never assume success" rule. 20 new tests (263/263 total), typecheck clean - covering the global gate, per-leg gates, SELL-before-BUY ordering, a failed SELL blocking its paired BUY while an unrelated ticker's BUY proceeds normally, all three audit-event outcomes, cash resizing (including the multi-leg cash-pool decrement), and confirming the adapter's return value has no state-machine side effects.
