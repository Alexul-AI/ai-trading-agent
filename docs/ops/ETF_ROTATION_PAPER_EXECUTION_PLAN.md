...
**Two integration hazards flagged in review of PR #43, not yet addressed - required before the execution-wiring PR, not before this one:**

1. **The monthly gate must be fully replaced, not layered.** `autopilotWorker.ts`'s `runEtfRotationCycle` (Stage 1, still unchanged by PR #43/#44) still calls the old `getLastRebalanceDateKey`/`isMonthlyRebalanceDate` pair to decide whether today is a rebalance day. The execution-wiring PR must replace this entirely with `isRebalanceMonthDone(state, monthKey)` (reading the full state via `getRebalanceState`/`readRebalanceStateStrict`) - if the old pair is left in place *alongside* the new `recordRebalancePlanned` (which already writes `lastRebalanceDateKey` at `"planned"`, before anything executes), the old check would see this month's date already recorded and treat `planned`/`executing`/even a stuck `failed_needs_review` as "already rebalanced," silently skipping any further attempt for the rest of the month.
2. **Fail-closed reads are now available but not yet consumed by any execution path**, because there is no execution path yet - `readRebalanceStateStrict` currently has exactly one caller (the read-only review endpoint above). The execution-wiring PR must use `readRebalanceStateStrict` (not the soft `getRebalanceState`/`getLastRebalanceDateKey`) for its own gate check, and must not start a new rebalance attempt when `corrupt: true` - it should treat that the same as finding a stuck `failed_needs_review` (alert, do not proceed, wait for the review endpoint / a human).
