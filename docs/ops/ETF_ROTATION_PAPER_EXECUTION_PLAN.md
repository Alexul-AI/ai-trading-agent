...
Nothing merged so far, including PR #47b, sets `AUTOPILOT_STRATEGY=etf_rotation` or `AUTOPILOT_EXECUTE_TRADES=true` in Render, and nothing touches `.env`/Render's environment from any of these PRs. The broker path this PR makes reachable in code stays dormant in the real deployment until both of those are explicitly changed there - a separate, future, explicitly-approved operational decision, not implied by this PR's merge. Per the standing project rule, no execution is ever performed on the user's behalf regardless of any of this.
