...
- **2026-07-13**: the sticky/no-auto-recovery design got its first real stress test in `backtest-portfolio.ts`'s next-open model, where it tripped once (2025-04-08) and then locked out new BUYs for 78% of a 406-day window, turning +20.66% into -9.68%. A backtest-only follow-up simulated several reset policies against that same trip (see `CLAUDE.md`'s "Strategy performance reality") - every one tested (reset after 1/3/5/10 trading days, or once drawdown recovered above -10%) turned the result positive (+5% to +8%) with **no worse max drawdown** than the current sticky policy. This doesn't change gate 5's requirement (still needs to fire and be handled at least once in real paper trading) - but it's a specific, evidence-backed reason to revisit whether "sticky forever until a human manually resets" is the right default before treating this gate as fully closed, not just "built and technically correct."
